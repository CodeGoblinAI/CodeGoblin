...
export class Service extends Context.Service<Service, Interface>()("@codegoblin/v2/Account") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const parseAuthContent = () => {
      try {
        return JSON.parse((process.env.CODEGOBLIN_AUTH_CONTENT ?? process.env.OPENCODE_AUTH_CONTENT) ?? "")
      } catch {}
    }

    const load: () => Effect.Effect<Writable, Error> = Effect.fnUntraced(function* () {
      if ((process.env.CODEGOBLIN_AUTH_CONTENT ?? process.env.OPENCODE_AUTH_CONTENT)) {
