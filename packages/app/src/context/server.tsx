    ...
    return {
      ...
      projects: {
        ...
        open(directory: string) {
          ...
          if (current.find((x) => pathKeysEqual(x.worktree, directory))) return
        ...
        close(directory: string) {
          ...
          setStore(
            "projects",
            key,
            current.filter((x) => !pathKeysEqual(x.worktree, directory)),
          )
        },
        expand(directory: string) {
          ...
          if (!key) return
          const current = store.projects[key] ?? []
          const index = current.findIndex((x) => pathKeysEqual(x.worktree, directory))
        ...
        move(directory: string, toIndex: number) {
          ...
          if (!key) return
          const current = store.projects[key] ?? []
          const fromIndex = current.findIndex((x) => pathKeysEqual(x.worktree, directory))
