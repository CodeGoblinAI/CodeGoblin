...
import { iife } from "@codegoblin/core/util/iife"
...
const USE_V2_TITLEBAR = true // redesign shipped in 0.2.x — no longer channel-gated
    ...
    >
      <Switch>
        <Match when={USE_V2_TITLEBAR}>
          {(_) => {
            // Minimal desktop bar for the three-pane design: window drag space,
            // the app menu (Windows/Linux), server status, and the update pill.
            // The old session tab strip / DEV badge / grid button are gone --
            // session navigation lives in the left sidebar now.
            const statusDir = createMemo(() => (params.dir ? decodeDirectory(params.dir) : undefined))
            return (
              <div
  ...
  return (
    <>
      {["beta", "dev"].includes(import.meta.env.VITE_OPENCODE_CHANNEL) && (
        <div class="inline-flex items-center gap-1 rounded-md border border-v2-border-border-focus bg-v2-background-bg-layer-01 px-2 py-px text-[11px] font-medium uppercase tracking-[0.12em] text-v2-text-text-accent font-mono shadow-[0_0_10px_color-mix(in_srgb,var(--v2-border-border-focus)_18%,transparent)]">
          <span class="text-v2-text-text-accent text-[9px]">&#9670;</span>
