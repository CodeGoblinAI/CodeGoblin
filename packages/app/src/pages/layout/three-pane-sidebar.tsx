...
import { clampNewSidebarWidth, getAvatarColors, useLayout, type LocalProject } from "@/context/layout"
...
export function ThreePaneSidebar(props: {
  ...
  return (
    <Show
      when={layout.newSidebar.opened()}
      fallback={
        <div class="flex h-full w-10 shrink-0 items-start justify-center border-r border-v2-border-border-weaker bg-v2-background-bg-deep pt-2">
          <IconButtonV2
            variant="ghost"
            size="large"
            icon={<IconV2 name="menu" />}
            onClick={() => layout.newSidebar.open()}
            aria-label="Open sidebar"
            title="Open sidebar"
            class="text-v2-icon-icon-base hover:text-v2-icon-icon-accent"
          />
        </div>
      }
    >
      <div class="relative h-full shrink-0" style={{ width: `${newSidebarWidth()}px` }}>
        <nav
          aria-label="Projects and chats"
          class="flex h-full w-full flex-col overflow-hidden border-r border-v2-border-border-weaker bg-v2-background-bg-deep"
        >
          {/* Header: logo + new chat */}
          <div class="flex h-11 shrink-0 items-center gap-2 border-b border-v2-border-border-weaker px-3">
            <CodeGoblinLogoMark size="sm" />
            <span class="min-w-0 flex-1 truncate text-[14px] font-semibold text-v2-text-text-base">CodeGoblin</span>
            <IconButtonV2
              variant="ghost"
              size="large"
              ...
              class="shrink-0 text-v2-icon-icon-base hover:text-v2-icon-icon-accent"
            />
            <IconButtonV2
              variant="ghost"
              size="large"
              icon={<IconV2 name="menu" />}
              onClick={() => layout.newSidebar.close()}
          ...
          <div class="min-h-0 flex-1 overflow-y-auto py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {/* Sessions */}
                      ...
                      <For each={group.items}>
                        {(item) => {
                          const title = createMemo(
                            () => sessionTitle(item.session.title) || item.session.id.slice(0, 8),
                          )
                          const isActive = () =>
                            ...
                            >
                              ...
                              >
                                <DropdownMenu.Trigger
                                  class="flex size-6 shrink-0 items-center justify-center rounded-[4px] border-0 bg-transparent text-v2-icon-icon-base opacity-75 transition-opacity cursor-default hover:bg-v2-overlay-simple-overlay-hover hover:text-v2-icon-icon-accent group-hover/chat:opacity-100 focus-visible:opacity-100"
            ...
            <div class="mt-1">
              ...
              <DropdownMenu gutter={6} placement="bottom-start">
                <DropdownMenu.Trigger
                  class={`${ROW_BASE} mt-1 h-8 w-full gap-1.5 px-3 text-v2-text-text-base hover:text-v2-text-text-accent`}
                >
                  <IconV2 name="folder-add-left" size="small" class="[&_[data-slot=icon-svg]]:text-v2-icon-icon-base" />
          ...
          {/* Footer */}
          <div class="flex shrink-0 items-center gap-1 border-t border-v2-border-border-weaker px-2 py-2">
            <IconButtonV2
              variant="ghost"
              size="large"
