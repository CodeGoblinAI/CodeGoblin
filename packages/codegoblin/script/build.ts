...
const createEmbeddedWebUIBundle = async () => {
  ...
  const { resolveViteNodeExecutable, viteBuildEnv, logViteNodeChoice, viteCliPath } = await import(
    "../../../script/resolve-vite-node.ts"
  )
  const node = resolveViteNodeExecutable()
  logViteNodeChoice(node)
  const viteCli = viteCliPath(appDir)
  await $`OPENCODE_CHANNEL=${Script.channel} ${node} ${viteCli} build`.cwd(appDir).env(viteBuildEnv(node))
...
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      outfile: `dist/${name}/bin/codegoblin`,
      execArgv: [`--user-agent=codegoblin/${Script.version}`, "--use-system-ca", "--"],
      windows: {},
    },
    files: embeddedFileMap ? { "codegoblin-web-ui.gen.ts": embeddedFileMap } : {},
    entrypoints: ["./src/index.ts", parserWorker, workerPath, ...(embeddedFileMap ? ["codegoblin-web-ui.gen.ts"] : [])],
    define: {
  ...
  if (item.os === process.platform && item.arch === process.arch && !item.abi) {
    const binaryPath = `dist/${name}/bin/codegoblin`
