...
```json
  "plugin": ["../packages/codegoblin/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate CodeGoblin server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/codegoblin/script/run-workspace-server
```

With the plugin install, you can now run CodeGoblin and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
