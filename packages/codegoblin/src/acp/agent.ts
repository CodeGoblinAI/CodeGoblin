...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@codegoblin/sdk/v2"
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `codegoblin auth login` in the terminal",
      name: "Login with CodeGoblin",
      id: "opencode-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // CodeGoblin stores files internally as { type: "file", url, filename, mime }.
