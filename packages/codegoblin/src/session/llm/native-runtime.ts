...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@codegoblin/llm"
import type { LLMClientShape } from "@codegoblin/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @codegoblin/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      ...
      // the @codegoblin/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
