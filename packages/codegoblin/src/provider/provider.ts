...
import * as ModelsDev from "@codegoblin/core/models-dev"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@codegoblin/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
...
export class ModelNotFoundError extends Schema.TaggedErrorClass<ModelNotFoundError>()("ProviderModelNotFoundError", {
  ...
  override get message() {
    const id = `${this.providerID}/${this.modelID}`
    // Local runtime models only exist while a GGUF is present on disk, so a
    // missing one is nearly always "the runtime isn't set up here" rather than a
    // typo. Saying so beats a bare not-found next to an apparently empty picker.
    if (this.providerID === "codegoblin") {
      return [
