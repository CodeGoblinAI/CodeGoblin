...
function rendererConfig(_config: TuiConfig.Resolved): CliRendererConfig {
  const mouseEnabled = !Flag.CODEGOBLIN_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.CODEGOBLIN_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CODEGOBLIN_DISABLE_TERMINAL_TITLE) return
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    if (!Flag.CODEGOBLIN_ALWAYS_NOTIFY_UPDATE) return
