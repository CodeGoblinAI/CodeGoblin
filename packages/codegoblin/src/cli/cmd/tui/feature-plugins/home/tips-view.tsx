...
const TIPS: Tip[] = [
  ...
  (shortcuts) => press(shortcuts.agentCycle(), "to cycle between Agent and Plan modes"),
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public upstream share link",
  ...
  (shortcuts) => `Use ${commandText("/model", shortcuts.modelList())} to see and switch between available AI models`,
  (shortcuts) => `Use ${commandText("/theme", shortcuts.themeList())} to switch between ${themeCount} built-in themes`,
  (shortcuts) => `Use ${commandText("/clear", shortcuts.sessionNew())} to start a fresh conversation session`,
  (shortcuts) => `Use ${commandText("/resume", shortcuts.sessionList())} to list, pin, and continue sessions`,
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.codegoblin/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.codegoblin/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.codegoblin/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.codegoblin/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent CodeGoblin from reading sensitive files",
  "Use {highlight}codegoblin run{/highlight} for non-interactive scripting",
  "Use {highlight}codegoblin --continue{/highlight} to resume the last session",
  "Use {highlight}codegoblin run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}codegoblin serve{/highlight} for headless API access to CodeGoblin",
  "Use {highlight}codegoblin run --attach{/highlight} to connect to a running server",
  "Run {highlight}/update{/highlight} in the TUI (or {highlight}codegoblin update{/highlight}) to get the latest version",
  "Run {highlight}codegoblin auth list{/highlight} to see all configured providers",
  "Run {highlight}codegoblin agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.codegoblin/themes/{/highlight} directory",
  ...
  "Run {highlight}codegoblin debug config{/highlight} to troubleshoot configuration",
