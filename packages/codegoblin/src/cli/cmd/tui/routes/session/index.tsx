...
import { SESSION_SIDEBAR_WIDTH, Sidebar } from "./sidebar"
...
export function Session() {
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}codegoblin -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      category: "Session",
      // Display setting, not a frequent action — reachable via keybind/settings, not ctrl+g.
      hidden: true,
      run: () => {
