...
describe("ServerAuth", () => {
  test("does not emit auth headers without a password", () => {
    Flag.CODEGOBLIN_SERVER_PASSWORD = undefined
    Flag.CODEGOBLIN_SERVER_USERNAME = "alice"

    expect(ServerAuth.header()).toBeUndefined()
    expect(ServerAuth.headers()).toBeUndefined()
  })

  test("defaults to the codegoblin username", () => {
    Flag.CODEGOBLIN_SERVER_PASSWORD = "secret"
    Flag.CODEGOBLIN_SERVER_USERNAME = undefined

    expect(ServerAuth.headers()).toEqual({
      Authorization: `Basic ${Buffer.from("codegoblin:secret").toString("base64")}`,
    })
  })
