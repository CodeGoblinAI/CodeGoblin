...
import { OpencodeClient, type Provider } from "@codegoblin/sdk/v2"
...
describe("run runtime boot", () => {
  ...
  test("falls back to default keybinds when config load fails", async () => {
    ...
    expect(formatBindings(result.inputNewline, result.leader)).toBe("ctrl+j, shift+return, ctrl+return, alt+return")
  })
