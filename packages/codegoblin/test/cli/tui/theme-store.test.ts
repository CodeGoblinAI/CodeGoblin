...
test("addTheme writes into module theme store", () => {
  ...
  expect(addTheme(name, DEFAULT_THEMES.codegoblin)).toBe(true)
...
test("addTheme keeps first theme for duplicate names", () => {
  ...
  const one = structuredClone(DEFAULT_THEMES.codegoblin)
  const two = structuredClone(DEFAULT_THEMES.codegoblin)
...
test("resolveTheme rejects circular color refs", () => {
  const item = structuredClone(DEFAULT_THEMES.codegoblin)
  item.defs = {
    ...item.defs,
    one: "two",
    two: "one",
