...
Delete `packages/codegoblin/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/codegoblin/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/codegoblin/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/codegoblin`
- TUI plugin loader tests in `packages/codegoblin` if runtime plugin API wiring changed
