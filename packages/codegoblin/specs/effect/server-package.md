...
- The server still lives in `packages/codegoblin`.
...
Until enough shared service code lives outside `packages/codegoblin`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/codegoblin`

It should not import `packages/codegoblin` services while `packages/codegoblin`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/codegoblin` implementation details.
