...
Replace runtime Bun shell template-tag usage in `packages/codegoblin/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/codegoblin/src` is removed except:
