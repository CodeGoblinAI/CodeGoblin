...
- Use `testEffect(...)` from `packages/codegoblin/test/lib/effect.ts` for tests that exercise Effect services, layers, runtime context, scoped resources, or platform integrations.
...
- Run tests from package directories such as `packages/codegoblin`; never run package tests from the repo root.
